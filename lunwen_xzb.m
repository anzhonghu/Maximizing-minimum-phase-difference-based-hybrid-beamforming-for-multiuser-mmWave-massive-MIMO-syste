...
SNR_db = -10:10:40;              %信噪比的设置
SNR = 10.^(SNR_db./10);
Loop = 1:length(SNR_db);         % Loop初始化 length表向量的长度
rate_ben = zeros(length(SNR_db), 1);
rate_opt = zeros(length(SNR_db), 1);
rate_gm = zeros(length(SNR_db), 1);  %未量化的施密特正交化
rate_ggm = zeros(length(SNR_db), 1); 
rate_beam=zeros(length(SNR_db), 1);
for Loop = 1:length(SNR_db) %信噪比循环
    for Loop_1 = 1 : N %循环次数
        ...
        for ii=1:K
            rate_ben(Loop,  1)=rate_ben(Loop, 1)+log2(1+power(ii,1)/(rao(ii,1)+zao(ii,1)/SNR(Loop)));
        end
        %%%%%%%%% gram正交化算法  未量化的
        U=zeros(M,K);
        W_gm=zeros(M,K);
        ...
        for ii=1:K
            ...
            rate_gm(Loop, 1)=rate_gm(Loop, 1)+log2(1+p_gm(ii,1)/(rao_gm(ii,1)+zao(ii,1)/SNR(Loop)));
        end
        %%%%%%%%% gram正交化算法  量化的
        F=zeros(M,co1);
        m=zeros(M,1);
        ...
        for ii=1:K
            ...
            rate_ggm(Loop, 1)=rate_ggm(Loop, 1)+log2(1+p_ggm(ii,1)/(rao_ggm(ii,1)+zao(ii,1)/SNR(Loop)));
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%  全数字
        W_opt = ((H'*H)\H')';
        p_opt = zeros(K, 1);
        ...
        for ii=1:K
            ...
            rate_opt(Loop, 1)=rate_opt(Loop, 1)+log2(1+p_opt(ii,1)/(rao_opt(ii,1)+zao(ii,1)/SNR(Loop)));
        end
        %%%%%%%%%%%%%%多用户受限反馈
        W_beam=zeros(M,K);
        ddd=zeros(co1,1);
        ...
        for ii=1:K
            ...
            rate_beam(Loop, 1)=rate_beam(Loop, 1)+log2(1+p_beam(ii,1)/(rao_beam(ii,1)+zao(ii,1)/SNR(Loop)));
        end
        disp([Loop, Loop_1])
    end
end
...
rate_ggm=rate_ggm/N;
plot(SNR_db,rate_opt,'--ko');
hold on
plot(SNR_db ,rate_gm,'--kd');
plot(SNR_db ,rate_ben,'-k*');
plot(SNR_db ,rate_ggm,'--k>');
plot(SNR_db,rate_beam,'--ks','Markersize', 12);
grid on
xlabel('信噪比/dB');
ylabel('和速率[bit/s/Hz]');
