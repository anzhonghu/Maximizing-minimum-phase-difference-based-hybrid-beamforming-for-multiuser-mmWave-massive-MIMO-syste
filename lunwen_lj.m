...
SNR = 10.^(SNR_db./10);
Ls = [3;4;5;6;7];
rate_ben = zeros(length(Ls), 1);
rate_opt = zeros(length(Ls), 1);
rate_gm = zeros(length(Ls), 1);
rate_beam=zeros(length(Ls), 1);
rate_ggm = zeros(length(Ls), 1);  
phase=cell(1,K);
phase_lh=cell(1,K);
aaa=cell(1,K);
fai=zeros(K,1);  %最大增益对应的相位
for Loop = 1:length(Ls)
    L = Ls(Loop, 1);
    for Loop_1 = 1 : N %循环次数
        for s=1:K
...
plot(Ls ,rate_ben,'-k*');
plot(Ls ,rate_ggm,'--k>');
plot(Ls,rate_beam,'--ks','Markersize', 12);
grid on
xlabel('路径数');
ylabel('和速率[bit/s/Hz]');
