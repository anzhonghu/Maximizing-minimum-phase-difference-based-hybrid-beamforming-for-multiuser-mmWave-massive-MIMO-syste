...
SNR = 10.^(SNR_db./10);
Ks = [2;4;6;8;10];
rate_ben = zeros(length(Ks), 1);
rate_opt = zeros(length(Ks), 1);
rate_gm = zeros(length(Ks), 1);
rate_beam=zeros(length(Ks), 1);
rate_ggm = zeros(length(Ks), 1); 
for Loop = 1:length(Ks)
    K = Ks(Loop, 1);
    phase=cell(1,K);
    phase_lh=cell(1,K);
    aaa=cell(1,K);
...
plot(Ks ,rate_ben,'-k*');
plot(Ks ,rate_ggm,'--k>');
plot(Ks,rate_beam,'--ks','Markersize', 12);
grid on
xlabel('用户数');
ylabel('和速率[bit/s/Hz]');
